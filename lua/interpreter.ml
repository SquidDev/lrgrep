...
let get_token =
  fun lexbuf -> Lexer_raw.token () lexbuf
...
let do_parse
  ...
  let rec loop : _ I.env -> _ I.checkpoint -> _ = fun env -> function
    ...
    | I.InputNeeded env' as cp ->
      ...
      Tokens.show_token token |> print_endline;
...
let get_error_message token env =
  match PE.run env |> List.find_map (Parse_errors.execute_error_message token) with
  | Some x -> x
  | None -> "Unhandled parse error at " ^ Tokens.show_token token

let parse input =
  let wrap_tok lexbuf tok = (tok, lexbuf.Lexing.lex_start_p, lexbuf.Lexing.lex_curr_p) in
  let lexbuf = Lexing.from_string input in
  let rec go env tok checkpoint =
    match checkpoint with
    | I.InputNeeded env ->
        let tok = Lexer_raw.token () lexbuf in
        I.offer checkpoint (wrap_tok lexbuf tok) |> go env tok
    | I.Shifting _ | I.AboutToReduce _ -> go env tok (I.resume checkpoint)
    | I.HandlingError _ -> 
      get_error_message tok env |> prerr_endline
    | I.Accepted () -> ()
    | I.Rejected -> assert false
  in
  match Parser_raw.Incremental.program lexbuf.lex_curr_p with
  | I.InputNeeded env as cp ->
      let token = Lexer_raw.token () lexbuf in
      I.offer cp (wrap_tok lexbuf token) |> go env token
  | _ -> assert false
...
let () =
  ...
  | Some file ->
    ...
    Lexing.from_string ~with_positions:true input |> do_parse Parser_raw.Incremental.program |> process_result;
    parse input;
    if close_ic then
      close_in_noerr ic
